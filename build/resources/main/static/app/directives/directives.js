angular.module('app.core').directive('uppercased', function() {
    return {
        require: 'ngModel',        
...
angular.module('app.core').directive('nospecialchar', function() {
	return {
		require : 'ngModel',
...
angular.module('app.core').directive('nospecialchar2', function() {
	return {
		require : 'ngModel',
		link : function(scope, element, attr, ngModelCtrl) {
			function fromUser(text) {
				if (text) {
					var transformedInput = text.replace(/[^a-z0-9 ñÑ]+/gi, '');
		...
		function($filter, $browser) {
			return {
				...
				link : function($scope, $element, $attrs, ngModelCtrl) {
					...
					// This runs when the model gets updated on the scope
					// directly and keeps our view in sync
					ngModelCtrl.$render = function() {
						$element.val($filter('tel')(ngModelCtrl.$viewValue,
								false));
					};
					...
					$element.bind('keydown', function(event) {
						...
						// If the keys include the CTRL, SHIFT, ALT, or META
						// keys, or the arrow keys, do nothing.
						// This lets us support copy and paste too
						if (key == 91 || (15 < key && key < 19)
								|| (37 <= key && key <= 40)) {
							return;
						}
						$browser.defer(listener); // Have to do this or
						// changes don't get picked
						// up properly
					});
...
angular.module('app.core').directive('validFile', function() {
	return {
		restrict : "A",
		require : "ngModel",
...
angular.module('app.core').directive('limitTo', [ function() {
	return {
		restrict : 'A',
...
angular.module('app.core').directive('loading', [ '$http', function($http) {
	return {
		restrict : 'A',
...
angular.module('app.core').directive('validPasswordC', function() {
	return {
		require : 'ngModel',
...
angular.module('app.core').directive('select2',
